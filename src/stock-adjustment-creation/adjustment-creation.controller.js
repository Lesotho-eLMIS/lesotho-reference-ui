...
(function() {
   ...
   */
    ...
    controller.$inject = ['$scope', '$state', '$stateParams', '$filter', 'confirmDiscardService', 'program', 'facility',
        ...
        'LotResource', '$q', 'editLotModalService', 'moment', 'rejectionReasonService',
    ...
    function controller($scope, $state, $stateParams, $filter, confirmDiscardService, program, facility,
                        ...
                        ADJUSTMENT_TYPE, UNPACK_REASONS, REASON_TYPES, STOCKCARD_STATUS, hasPermissionToAddNewLot,
                        LotResource, $q, editLotModalService, moment, rejectionReasonService,
                        receivingAddDiscrepancyModalService, complaintFormModalService, suppliers, ReferenceNumbers,
                        facilityWithType, QUANTITY_UNIT, quantityUnitCalculateService) {
        var vm = this,
            previousAdded = {};
     ...
     */
        ...
        // To store shipment rejection reasons.
        vm.rejectionReasons = [];
     ...
     */
        ...
        vm.addComplaintOnModal = function(itemTimestamp) {
            complaintFormModalService
                .show(itemTimestamp, program, facility, orderableGroups, hasPermissionToAddNewLot)
                .then(function() {
                    $stateParams.noReload = true;
                    vm.cacheDraft();
                    //Only reload current state and avoid reloading parent state
     ...
     */
        ...
        vm.validateCartonNumberRange = function(lineItem) {
            if (adjustmentType.state === 'receive') {
                if (!lineItem.hasOwnProperty('totalCartonNumber') ||
                    lineItem.individualCartonNumberRange > lineItem.totalCartonNumber ||
                    isEmpty(lineItem.totalCartonNumber) ||
                    lineItem.individualCartonNumberRange === 0) {

                    lineItem.$errors.cartonsInvalid = messageService.get('stockAdjustmentCreation.cartonsInvalidError');
                } else if (lineItem.individualCartonNumberRange > 0 &&
                    lineItem.individualCartonNumberRange <= lineItem.totalCartonNumber &&
                    lineItem.individualCartonNumberRange >= lineItem.individualCartonNumber) {

                    lineItem.$errors.cartonsInvalid = false;
                    let cartonNumber = lineItem.individualCartonNumber + ' to ' +
                        lineItem.individualCartonNumberRange + ' of ' + lineItem.totalCartonNumber;
        ...
        vm.validateCartonNumber = function(lineItem) {
            ...
            if (adjustmentType.state === 'receive') {
                if (!lineItem.hasOwnProperty('totalCartonNumber') ||
                    lineItem.individualCartonNumber > lineItem.totalCartonNumber ||
                    isEmpty(lineItem.totalCartonNumber) ||
                    lineItem.individualCartonNumber === 0) {

                    lineItem.$errors.cartonsInvalid = messageService.get('stockAdjustmentCreation.cartonsInvalidError');
                } else if (lineItem.individualCartonNumber > 0 &&
                    lineItem.individualCartonNumber <= lineItem.totalCartonNumber) {
   ...
   vm.submit = function () {
     ...
     */
        ...
        function filterFacilities() {
            ...
            if (adjustmentType.state === 'receive') {
                vm.srcDstAssignments = vm.srcDstAssignments.filter(
                    item => item.type && item.type.name !== 'Unserviceable'
                );
            }
      ...
      vm.showDeliveryNoteAttributes =
        ...
        adjustmentType.state === ADJUSTMENT_TYPE.ADJUSTMENT.state;
            // (facility.type.code === 'quarantine' || facility.type.code === 'unserviceable')
            vm.servicePointUser =
                adjustmentType.state === ADJUSTMENT_TYPE.RECEIVE.state &&
